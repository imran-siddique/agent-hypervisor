...
class TestQuarantine:
    ...
    def test_quarantine_escalation(self):
        mgr = QuarantineManager()
        first = mgr.quarantine("agent-a", "sess-1", QuarantineReason.BEHAVIORAL_DRIFT)
        second = mgr.quarantine(
            "agent-a", "sess-1",
            QuarantineReason.LIABILITY_VIOLATION,
            details="Additional violation",
        )
        # Should return same record (escalated)
    ...
    def test_quarantine_with_forensic_data(self):
        ...
        record = mgr.quarantine(
            "agent-a", "sess-1",
            QuarantineReason.RING_BREACH,
            forensic_data={"drift_score": 0.9, "actions": ["write", "delete"]},
        )
