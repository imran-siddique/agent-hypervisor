...
class TestFanOut:
    ...
    async def test_majority_policy_succeeds(self, steps):
        ...
        executors = {s.step_id: mostly_succeed for s in steps}
        result = await fan.execute(group.group_id, executors)

        assert result.policy_satisfied  # 2/3 > 50%
    ...
    async def test_any_policy_succeeds(self, steps):
        ...
        executors = {s.step_id: mostly_fail for s in steps}
        result = await fan.execute(group.group_id, executors)

        assert result.policy_satisfied  # 1/3 >= 1
    ...
    def test_group_check_policy_any_empty(self):
        ...
        assert not group.check_policy()  # 0 successes < 1
...
class TestCheckpoints:
    ...
    def test_get_checkpoint(self):
        ...
        assert ckpt.state_snapshot["pod_count"] == 3
    ...
    def test_replay_plan(self):
        mgr = CheckpointManager()
        mgr.save("saga-1", "s1", "Step 1")
        mgr.save("saga-1", "s3", "Step 3")

        plan = mgr.get_replay_plan("saga-1", ["s1", "s2", "s3", "s4"])
        assert plan == ["s2", "s4"]
    ...
    def test_parse_fan_out_invalid_branch(self):
        ...
        with pytest.raises(SagaDSLError, match="not a valid step"):
            parser.parse({
                ...
                "fan_out": [{"policy": "all_must_succeed", "branches": ["a", "nonexistent"]}],
            })
    ...
    def test_parse_fan_out_too_few_branches(self):
        ...
        with pytest.raises(SagaDSLError, match="at least 2"):
            parser.parse({
                ...
                "fan_out": [{"policy": "all_must_succeed", "branches": ["a"]}],
            })
